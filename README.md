...
The Kaiser filter, depending on the desired parameters, produces an order $N$ of the filter . Its resulting group delay is a direct consequence of that order, and is equal to

$$
\tau_g = \frac{N-1}{2}
$$

It represents the constant delay introduced by the filter due to its symmetry and linear-phase property, ensuring processing without phase distortion. Since the filter requirements are relatively strict, the filter order \( N \) and the resulting delay are relatively high:  

$$
N = 2570 \quad\quad\quad \tau_{\text{samples}} = 1285 \quad\quad\quad \tau_{\text{time}} = \frac{\tau_g}{f_c} \approx 2.5 \text{sec}
$$

Filtering can be implemented using two main approaches:
1. In linear convolution, this delay explicitly manifests as a temporal shift of the filtered signal relative to the input, with an output extension caused by the initial and final samples generated by the convolution process. 
2. In circular convolution, the delay, while still present, is wrapped cyclically within the implicit periodicity, resulting in interference between the signal's edges, which necessitates adjustments for non-periodic applications.
